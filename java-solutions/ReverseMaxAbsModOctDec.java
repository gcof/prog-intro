...
public class ReverseMaxAbsModOctDec {
    ...
    static int getMax(int a, int b) {
        return Math.abs(a) % MOD > Math.abs(b) % MOD ? a : b;
    }
    ...
    static int[][] evalColRow(int[][] matrix) {
        ...
        for (int i = 0; i < matrix.length; i++) {
            for (int j = 0; j < matrix[i].length; j++) {
                int toAdd = matrix[i][j];
                cols[j] = getMax(cols[j], toAdd);
                rows[i] = getMax(rows[i], toAdd);
            }
        }
    ...
    static void printMatrixReversedSumMod(int[][] matrix) {
        ...
        for (int i = 0; i < matrix.length; i++) {
            for (int j = 0; j < matrix[i].length; j++) {
                int toOut = getMax(rowSum[i], colSum[j]);
                if (toOut < 0) {
                    // System.err.print(Integer.toOctalString((int)toOut) + "o ");
                    System.out.print(Integer.toOctalString((int)toOut) + "o ");
                }else {
                    // System.err.print(int.toOctalString(toOut) + "o ");
                    System.out.print(Integer.toOctalString(toOut) + "o ");
                }  
            }
            // System.err.println();
            System.out.println();
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            try {
                ...
                System.err.println("Error while closing scanner: " + e.getMessage());
            }
        }
